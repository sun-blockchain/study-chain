...
class FabricClient {
  ...
  async getGenesisBlock(channel) {
    ...
    if (!genesisBlock) {
      ...
      while (neworderer != null) {
        ...
        genesisBlock = await this.getGenesisBlockFromOrderer(
          channel,
          neworderer
        );
        if (genesisBlock) {
          ...
          logger.error(
            'Failed to get genesis block with ' + neworderer.getName()
          );
  ...
  async getGenesisBlockFromOrderer(channel, orderer) {
    try {
      ...
      logger.error(
        'Failed to get genesis block with ' + orderer
          ? orderer.getName()
          : 'null'
      );
      return null;
    ...
    for (const mspid in channel._discovery_results.orderers) {
      ...
      for (let i = 0; i < endpoints.length; i++) {
        ...
        if (orderer.getName().split(':')[0] === value.host) {
          ...
          channel._discovery_results.orderers[mspid].endpoints[
            i
          ]._offline = true;
          break;
        }
